...
def main():
    ...
    resource_path = (
        os.path.abspath(os.path.dirname(sys.modules[__name__].__file__)) + "/resources"
    )
    ...
    parser.add_argument(
        "--port", help="Bind port to listen for requests", default=bind_port
    )
    parser.add_argument(
        "--proxy_api_url", help="URL to API for Swagger", default=proxy_api_url
    )
    parser.add_argument(
        "--resource-path", help="Path to API resource modules", default=resource_path
    )
